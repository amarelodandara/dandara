...
import {
  ...
  listItems,
  splitLabel,
} from "@/components/graph/graph-frame";
import {
  fadeUp,
  staggerList,
  toneClass,
  type GraphPalette,
} from "@/components/graph/graph-motion";
...
function typeOf(token: string): ChangeType | undefined {
  ...
  if (key === "add" || key === "added" || key === "launched" || key === "+") {
    return "add";
  }
  if (
    key === "change" ||
      ...
      <GraphBody className="flex flex-col gap-4">
        ...
        >
          {changes.map((change, index) => {
            ...
            return (
              <motion.li
                className="grid grid-cols-[1.25rem_5.5rem_minmax(0,1fr)] items-baseline gap-x-3 @max-md:grid-cols-[1.25rem_minmax(0,1fr)]"
                key={`${index}-${type}`}
                variants={item}
              >
                <span
                ...
                <span className="text-graph-muted @max-md:col-start-2">
                  {label[type]}
                </span>
                <GraphProse
                  className={cn(
